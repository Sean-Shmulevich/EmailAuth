import { g as generateRandomString$1, d as debug, p as prismaClient } from "./debug.js";
...
const generateScryptHash = async (s) => {
  const salt = generateRandomString$1(16);
...
class Auth {
  ...
  constructor(config) {
    ...
    this.generateUserId = config.generateCustomUserId ?? (() => generateRandomString$1(15));
    this.env = config.env;
  ...
  generateSessionId = () => {
    const sessionId = generateRandomString$1(40);
...
class LuciaTokenError extends Error {
  message;
...
const idToken = (auth2, name, options) => {
  return {
    ...
    validate: async (token) => {
      try {
        const key = await auth2.useKey(name, token, null);
    ...
    getUserTokens: async (userId) => {
      const keys = await auth2.getAllUserKeys(userId);
    ...
    invalidate: async (token) => {
      await auth2.deleteKey(name, token);
    },
    invalidateAllUserTokens: async (userId) => {
      try {
        const keys = await auth2.getAllUserKeys(userId);
        const targetKeys = keys.filter((key) => key.providerId === name);
        await Promise.all(targetKeys.map((key) => auth2.deleteKey(key.providerId, key.providerUserId)));
      } catch (e) {
        const error = e;
        if (error.message === "AUTH_INVALID_USER_ID")
          return;
...
const emailVerificationToken = idToken(auth, "email_verification", {
  expiresIn: 60 * 60
});
const passwordResetToken = idToken(auth, "email_reset", {
  expiresIn: 60 * 60
});
export {
  LuciaError as L,
  auth as a,
  emailVerificationToken as e,

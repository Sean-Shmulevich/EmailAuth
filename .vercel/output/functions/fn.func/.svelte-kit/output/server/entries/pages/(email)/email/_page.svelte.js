import { c as create_ssr_component, g as add_attribute, r as each, j as escape } from "../../../../chunks/index2.js";
import "devalue";
...
<div class="my-4 grid grid-cols-1 divide-y">${data.emails.length < 1 ? `<p class="my-2 text-center text-zinc-500">No emails were found</p>` : ``}
    ...
    return `<div class="py-4"><div class="flex place-content-between text-sm"><p>To: ${escape(email.toAddress)}</p>
			...
			<a class="mt-2 font-medium hover:underline"${add_attribute("href", `/email/${email.emailId}`, 0)}>${escape(email.subject)}</a>
		</div>`;
  })}</div>`;
});
export {
